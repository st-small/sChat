...
//  sChat
//
//  Created by Stanly Shiyanovskiy on 01.07.2020.
...
public final class ChatsViewController: MessagesViewController {
    ...
    public init(user: SUser, chat: SChat) {
        self.user = user
        self.chat = chat
    ...
    private func sendPhoto(image: UIImage) {
        StorageService.shared.uploadImageMessage(photo: image, to: chat) { (result) in
            ...
            case .success(let url):
                var message = SMessage(user: self.user, image: image)
